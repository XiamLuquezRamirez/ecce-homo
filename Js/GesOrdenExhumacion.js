$(document).ready(function () {
    ...
    $("#menu_serv_exhuma").addClass("active");

//   $("#CbProyect,#CbDepen,#CbResp,#CbElab,#CbContratista,#CbDepa,#CbRubPres,#CbMod,#CbTipCont").select2();


    $("#txt_fecha_nac,#txt_fecha_Cre, #txt_fecha_fall,#txt_fecha_CreFact,#txt_fecha_CreCos,#txt_fecha_Cons,#txt_FecInhu").datepicker({
        format: 'yyyy-mm-dd',
        autoclose: true,
        todayBtn: 'linked',
        todayHighlight: true,
    ...
    $("#Cbcemen,#TipProp,#CbFacNombCos,#CbFacNombFact,#CbPosic,#CbFpago,#CbConsigConst").selectpicker();
    ...
    $.extend({
        ...
        busqCont: function (val) {


            var datos = {
                ope: "VentContratosExhumaciones",
                bus: val
            }
            ...
            $.ajax({
                ...
                dataType: 'JSON',
                success: function (data) {
                    $("#tab_Contratos").show(100).html(data['tab_cli']);
                },
                error: function (error_messages) {
                    alert('HA OCURRIDO UN ERROR');
                }
        ...
        SelContrato: function (val) {
            var par = val.split("//");

            $("#txt_id_contrato").val(par[0]);
            $("#txt_contrato").val(par[1]);
            $("#Cbcemen").selectpicker("val", par[2]);
            $("#txt_NomFalle").val(par[3]);
            $("#txt_FecInhu").val(par[4]);
            $("#txt_Ubic").val(par[5]);

            $('#contratos').modal('toggle');
        ...
        InfCem: function () {
            if ($("#TipProp").val() === "Arriendo") {
                if ($("#Cbcemen").val() === "NUEVO") {
                    $('#txt_iden').val("892300318");
                } else {
                    $('#txt_iden').val("892300318-0");
                }
            }
        },
        ...
        editExhu: function (cod) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#TipProp").selectpicker("val", data["tprop"]);
                    $("#txt_FecInhu").val(data['fecinhuma']);
                    $("#txt_fecha_Exh").val(data['fechexhu']);
                    $("#txt_obser").val(data['obser']);
                    $("#txt_DirAut").val(data['dir_aut']);
                    $("#txt_TelAut").val(data['tel_aut']);
                    $("#txt_Trasla").val(data['txt_Trasla']);

                    $("#txt_obser").val(data['observa']);
                    $("#txt_NomJefe").val(data['jefe']);
                    $("#txt_iden").val(data['cedcli']);
                    $("#txt_NomCli").val(data['nomcli']);
                    $("#txt_Dir").val(data['dir']);
                    $("#txt_Tel").val(data['tel']);
                    $("#txt_id_cont").val(data['idcont']);
                    $("#txt_NomAutori").val(data['autori']);
                    ...
                    //$('#CbUbic').val(data["ido"]).change();
                    $("#CbPosic").selectpicker("val", data["posi"]);
                    $("#Cbcemen").selectpicker("val", data["cement"]);
                }
            });
            // $.editExhu(cod);
            ...
            $('#atitulo').show(100).html("<a href='#tab_02' data-toggle='tab' id='atitulo'>Editar Exhumaci&oacute;n</a>");


        },
        VerExhu: function (cod) {
            $('#btn_impri').show();
            ...
            $("#tab_02").addClass("active in");
            $("#tab_02_pp").addClass("active in");
            $('#atitulo').show(100).html("<a href='#tab_02' data-toggle='tab' id='atitulo'>Ver  Exhumaci&oacute;n</a>");
        ...
        AbrirContrato: function () {
            ...
            $.ajax({
                ...
                dataType: 'JSON',
                success: function (data) {
                    $("#tab_Contratos").html(data['tab_cli']);
                },
                error: function (error_messages) {
                    alert('HA OCURRIDO UN ERROR');
                }
...
//                    $("#CbCem").html(data['ceme']);
        ...
        conse: function () {
            ...
            if (text === "Crear Orden de Exhumación") {
        ...
        PrintExhu: function (val) {
            window.open("PDF_OrdenExhumacion.php?id=" + val + "", '_blank');
        },
...
//                        $("#txt_TelCliFact").val(data['tel']);
    ...
    $("#txt_cod").on("change", function () {
        ...
        $.ajax({
            ...
            dataType: 'JSON',
            success: function (data) {
                $('#txt_DAct').val(data['desc_actividad']);
                $('#txt_id_Act').val(data['id_actividad']);
            },
            error: function (error_messages) {
                alert('HA OCURRIDO UN ERROR');
            }
    ...
    $("#btn_cancelar").on("click", function () {
        window.location.href = 'GesOrdenExhum.php';
    });

    $("#btn_impri").on("click", function () {
        window.open("PDF_OrdenExhumacion.php?id=" + $('#txt_id').val() + "", '_blank');
    });
    ...
    $("#btn_guardar").on("click", function () {
        ...
        if ($('#txt_Cod').val() === "") {
            alert("Ingrese el Codigo de la Exhumacion");
            $('#txt_Cod').focus();
            return;
        }

        if ($("#TipProp").val() === "Propio") {
            if ($('#txt_iden').val() === "") {
        ...
        if ($("#txt_nuevo").val() === "NO") {
            if ($('#CbUbic').val() === "" && $('#TipProp').val() === "Propio") {
        ...
        var tip = "";
        var texubi = "";
        if ($("#txt_nuevo").val() === "NO" && $('#TipProp').val() === "Propio") {
        ...
        var datos = "codigo=" + $("#txt_Cod").val() + "&fcreac=" + $("#txt_fecha_Cre").val() + "&TipProp=" + $("#TipProp").val()
                + "&txt_Ciuda=" + $("#txt_Ciuda").val() + "&txt_iden=" + $("#txt_iden").val() + "&ubi=" + Ubic + "&tip=" + tip
                + "&CbPosic=" + $("#CbPosic").val() + "&txt_NomFalle=" + $("#txt_NomFalle").val() + "&Cbcemen=" + $("#Cbcemen").val()
                + "&txt_FecInhu=" + $("#txt_FecInhu").val() + "&txt_fecha_Exh=" + $("#txt_fecha_Exh").val() + "&txt_nuevo=" + $("#txt_nuevo").val()
                + "&txt_NomAutori=" + $("#txt_NomAutori").val() + "&txt_obser=" + $("#txt_obser").val()
                ...
                + "&txt_contrato=" + $("#txt_contrato").val() + "&txt_id_contrato=" + $("#txt_id_contrato").val() 
                + "&txt_DirAut=" + $("#txt_DirAut").val() + "&txt_TelAut=" + $("#txt_TelAut").val() + "&txt_Trasla=" + $("#txt_Trasla").val();
