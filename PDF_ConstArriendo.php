...
$link=conectar();
    ...
    $consulta = "SELECT * FROM constanciasarriendo where id_constan='".$_REQUEST["id"]."'";
    //echo $consulta;
...
if (mysqli_num_rows($resultado) > 0) {
    while ($fila = mysqli_fetch_array($resultado)) {
             ...
             $ciudad = $fila["ciudad"];
             $iden_constan = $fila["iden_constan"];
             $inde_cli = $fila["iden_constan"];
             $nom_cli = $fila["nom_constan"];
             $dir_cli = $fila["dircons"];
...
//*****
 ...
 $this->SetFont('Arial', '', 9);
        ...
        $this->SetXY(10, $Y);
        $item = $item+1;
	$this->Cell(15, 5, $item, 0);
        $this->MultiCell(130, 5, utf8_decode($GLOBALS['concepto']), 0);
        $H = $this->GetY();
        $height= $H-$Y;
        $this->SetXY(175,$Y);
