...
$consulta = "SELECT * FROM constanciasventas const LEFT JOIN clientes cli ON const.iden_constan=cli.inde_cli where id_constan='" . $_REQUEST["id"] . "'";
//echo $consulta;
...
//*****
    ...
    function TablaConceptos() {
        ...
        $consulta = "SELECT * FROM constanciasventas_detalles WHERE id_cosntancia='" . $_REQUEST["id"] . "'";
        //echo $consulta;
        $item = 0;
        ...
        if (mysqli_num_rows($resultado) > 0) {
            while ($fila = mysqli_fetch_array($resultado)) {
                ...
                $height = $H - $Y;
                $this->SetXY(175, $Y);
                $this->Cell(40, $height, '$ ' . number_format($fila["valor"], 2, ",", "."), 0);
                $this->Ln(8);
