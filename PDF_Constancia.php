...
$consulta = "SELECT * FROM constancias  where id_constan='" . $_REQUEST["id"] . "'";
//echo $consulta;
...
//*****
    ...
    function TablaConceptos() {
        ...
        $this->Cell(70, 8, utf8_decode('Descripción'), 0);
        $this->Cell(60, 8, utf8_decode('Observación'), 0);
        $this->Cell(20, 8, utf8_decode('Cantidad'), 0);
        ...
        if (mysqli_num_rows($resultado) > 0) {
            while ($fila = mysqli_fetch_array($resultado)) {
                ...
                $this->MultiCell(70, 3, utf8_decode(acentos($fila['descr'])), 0);
                $H = $this->GetY();
                $height = $H - $Y;
                $this->SetXY(96, $Y);
                $this->MultiCell(60, $height, utf8_decode(acentos($fila['obs'])), 0);
                $H = $this->GetY();
                $height = $H - $Y;
                $this->SetXY(153, $Y);
                $this->Cell(20, $height, $fila['can'], 0, 0, 'C');
                $this->SetXY(175, $Y);
                $this->Cell(40, $height, '$ ' . number_format($fila["val"], 2, ",", "."), 0);
                $this->Ln(8);
