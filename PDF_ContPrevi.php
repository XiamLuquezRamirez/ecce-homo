...
$consulta = "SELECT * FROM contrato_prevision previ LEFT JOIN clientes cli ON previ.id_titu=cli.inde_cli where previ.id_contr='" . $_REQUEST["id"] . "'";
$resultado = mysqli_query($link,$consulta);
if (mysqli_num_rows($resultado) > 0) {
    while ($fila = mysqli_fetch_array($resultado)) {
        $ncontrato = $fila["ncontrato"];
        $fecha_cre = $fila["fecha_cre"];
        $ciudad = $fila["ciudad"];
        $depar = $fila["depar"];
        $plan = $fila["plan"];
        $tipo_vinc = $fila["tipo_vinc"];
        ...
        $nom_emple = acentos($fila["nom_emple"]);
        $dir_emple = acentos($fila["dir_emple"]);
        $email_cli = acentos($fila["email_cli"]);
        $ciud_emple = $fila["ciud_emple"];
        $depar_emple = $fila["depar_emple"];
        $tel_emple = $fila["tel_emple"];
        $val_anual = number_format($fila["val_anual"], 2, ",", ".");
        $val_mes = number_format($fila["val_mes"], 2, ",", ".");
        $form_pago = $fila["form_pago"];
        $fech_ini = $fila["fech_ini"];
...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->SetXY(110, 14);
        $this->cell(0, 25, utf8_decode('ANUAL EXEQUIAL'), 0, 2, 'C');
    }
    ...
    function TablaBasica() {
        ...
        $this->SetXY(10, 88);
        $this->SetFont('Arial', '', 9);
        $this->Cell(100, 8, utf8_decode('Diocesano  ' . $GLOBALS['pdio'] . '       Familia Integral  ' . $GLOBALS['pfam'] . '         Dorado  ' . $GLOBALS['pdor'] . ''), 0, 0, 'L');
        $this->Line(26, 93, 33, 93);
        $this->Line(58, 93, 65, 93);
        $this->Line(78, 93, 84, 93);
        ...
        $this->SetFont('Arial', 'B', 7);
        $this->SetXY(10, 100);
        $this->Cell(30, 8, utf8_decode('Número de Documento.'), 0, 'L');
        $this->SetXY(10, 104);
        $this->SetFont('Arial', '', 9);
        $this->Cell(30, 8, utf8_decode($GLOBALS['id_titu']), 0, 'L');
        $this->SetXY(55, 100);
        ...
        $this->SetFont('Arial', '', 9);
        $this->SetXY(55, 104);
        $this->Cell(0, 8, utf8_decode($GLOBALS['nomb_titu']), 0, 'L');
    ...
    function TablaGrupos() {
        ...
        $this->SetFont('Arial', 'B', 9);
        $this->SetFillColor(205, 205, 205); // establece el color del fondo de la celda (en este caso es AZUL
        $this->Cell(0, 5, utf8_decode('GRUPO FAMILIAR BÁSICO'), 1, 2, 'C', True); // en orden lo que informan estos parametros es:
        ...
        $consulta = "SELECT * FROM  grupo_fambasico WHERE id_cont='" . $_REQUEST["id"] . "'";
        //echo $consulta;
        $item = 0;
        ...
        if (mysqli_num_rows($resultado) > 0) {
            while ($fila = mysqli_fetch_array($resultado)) {
                ...
                $this->Cell(20, 5, $fila['ident'], 0);
                $this->MultiCell(70, 5, utf8_decode(acentos($fila['nombre'])), 0);
                $H = $this->GetY();
                $height = $H - $Y;
                $this->SetXY(105, $Y);
                $this->Cell(25, $height, utf8_decode(acentos($fila['parent'])), 0, 0);
                $this->SetXY(130, $Y);
                $this->Cell(25, $height, utf8_decode($fila['estad']), 0);
                $this->SetXY(155, $Y);
                $this->Cell(10, $height, utf8_decode($fila['edad']), 0, 0, 'C');
                $this->SetXY(165, $Y);
                $this->Cell(0, $height, utf8_decode(acentos($fila['ciudad'])), 0);
                $this->Ln(7);
        ...
        $this->SetFont('Arial', 'B', 9);
        $this->SetFillColor(205, 205, 205); // establece el color del fondo de la celda (en este caso es AZUL
        $this->Cell(0, 5, utf8_decode('GRUPO FAMILIAR SECUNDADIO Y/O ADICIONALES'), 1, 2, 'C', True); // en orden lo que informan estos parametros es:
        // $this->Line(10, 120.5, 200, 120.5);
        $this->Ln(1);
        $this->SetFont('Arial', 'B', 8);
        $this->SetFillColor(205, 205, 205);
        ...
        $consulta = "SELECT * FROM  grupo_famsecundario WHERE id_cont='" . $_REQUEST["id"] . "'";
        //echo $consulta;
        $item = 0;
        $resultado = mysqli_query($GLOBALS['link'],$consulta);
        $Y = $Y + 17;
        ...
        if (mysqli_num_rows($resultado) > 0) {
            while ($fila = mysqli_fetch_array($resultado)) {
                ...
                $this->Cell(10, $height, utf8_decode($fila['edad']), 0, 0, 'C');
                $this->SetXY(165, $Y);
                $this->Cell(0, $height, utf8_decode($fila['ciudad']), 0);
                $this->Ln(7);
