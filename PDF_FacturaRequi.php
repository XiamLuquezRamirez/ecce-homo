...
$consulta = "SELECT * FROM facturas_requi fat LEFT JOIN requisiciones requi ON fat.requi=requi.id_req where fat.id_fact='" . $_REQUEST["id"] . "'";
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_array($resultado)) {
        ...
        $ident_difunto = $fila["idfall_req"];
        $nomb_difunto = acentos($fila["nomb_difunto"]);
        $fech_muert = $fila["fech_muert"];
    }
}
...
//*****
        ...
        $this->Cell(20, 8, utf8_decode('DAT. DIFUNTO - Cédula: ' .$GLOBALS['ident_difunto'].' - Nombre: '.$GLOBALS['nomb_difunto']), 0, 'L');
    ...
    function TablaConceptos() {
        ...
        $consulta = "SELECT
          ...
          FROM
            ...
            detalles_facturarequi conc

            LEFT JOIN servicios serv

              ON conc.detalle = serv.id_serv
        ...
        if (mysqli_num_rows($resultado) > 0) {
            ...
            while ($fila = mysqli_fetch_array($resultado)) {
                ...
                $this->MultiCell(110, 3, utf8_decode(acentos($fila['descr'])), 0);
