...
$consulta = "SELECT rec.conse conse,rec.fecha fecha, rec.valor valor, rec.nombre nombre,"
        . " rec.iden iden,rec.valletra valletra,rec.concepto concepto,YEAR(rec.fecha) aniorec,"
        . "rec.cuotames cuotames,rec.fpago fpago,rec.ncheque ncheque,rec.nbanco nbanco,anio.anio anio"
        . " FROM recibos rec left join anios_contrato anio on rec.anio=anio.id where rec.id='" . $_REQUEST["id"] . "'";
//echo $consulta;
...
//*****
        ...
        $this->SetXY(120, 43);
        $this->Cell(20, 8, 'C.C: ' . utf8_decode($GLOBALS['iden']), 0, 'L');
...
//*****
        ...
        //*****
        $this->SetXY(10, 57);
        $this->Cell(20, 8, 'Por concepto de Pago: ' . $GLOBALS['concepto'], 0, 0, 'L');

        $this->Line(10, 64.5, 200, 64.5);
        //*****
        $this->SetXY(10, 64);
        $this->Cell(20, 8, 'Cuotas: ' . $GLOBALS['cuotames'] . "-" . $GLOBALS['anio'], 0, 0, 'L');
