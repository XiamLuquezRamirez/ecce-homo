...
$cad = "<table class=\"table table-striped table-bordered table-advance table-hover\">"
        ...
        . "<tr>"
        . "<th>"
        . "<i class=\"fa fa-angle-right\"></i> Consecutivo"
        . "</th>"
        . "<th>"
...
if ($busq != "") {
    ...
    $consulta = "SELECT * FROM  constancias  WHERE ";
    ...
    $consulta .= " AND estado='ACTIVO' order by nom_constan ASC LIMIT " . $regemp . "," . $regmos;
} else {

    $consulta = "SELECT * FROM constancias WHERE estado='ACTIVO' order by nom_constan ASC  LIMIT " . $regemp . "," . $regmos;
}
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_array($resultado)) {
        ...
        $cad .= "<tr>"
                ...
                . "</td>"
                . "<td class=\"highlight\">"
                . acentos($fila["iden_constan"] . " - " . $fila["nom_constan"]) . ""
                . "</td>"
                . "<td class=\"highlight\">"
                . "$ " . number_format($fila["valor"], 2, ",", ".") . ""
                . "</td>"
                . "<td class=\"highlight\">"
                . "<a  onclick=\"$.editConst('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                . "<i class=\"fa fa-edit\"></i> Editar</a>"
                . "<a   onclick=\"$.VerConst('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                . "<i class=\"fa fa-search\"></i> Ver</a>"
                . "<a onclick=\"$.deletConst('" . $cod . "')\" class=\"btn default btn-xs red\">"
                . "<i class=\"fa fa-trash-o\"></i> Eliminar</a>"
                . "<a onclick=\"$.PrintConst('" . $cod . "')\" class=\"btn default btn-xs blue-dark\">"
...
$consulta = "SELECT count(*) as conta FROM constancias WHERE estado='ACTIVO'";
