...
if ($busq != "") {
    ...
    $consulta = "SELECT inh.id id, inh.conse conse, inh.fecha fec, CONCAT(cli.inde_cli,'-',cli.nom_cli) titu, inh.falle fall, inh.ubic ub  FROM inhumaciones inh LEFT JOIN clientes cli ON inh.titular=cli.inde_cli WHERE ";
    ...
    $consulta .= " AND inh.estado='ACTIVO' GROUP BY cli.inde_cli order by cli.nom_cli ASC LIMIT " . $regemp . "," . $regmos;
} else {

    $consulta = "SELECT inh.id id, inh.conse conse, inh.fecha fec, CONCAT(cli.inde_cli,'-',cli.nom_cli) titu, inh.falle fall, inh.ubic ub  FROM inhumaciones inh LEFT JOIN clientes cli ON inh.titular=cli.inde_cli WHERE  inh.estado='ACTIVO' GROUP BY cli.inde_cli order by cli.nom_cli ASC  LIMIT " . $regemp . "," . $regmos;
}
//echo $consulta;
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_array($resultado)) {
        ...
        $cad .= "<tr>"
                ...
                . "</td>"
                . "<td class=\"highlight\">"
                . "<a  onclick=\"$.editInhu('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                . "<i class=\"fa fa-edit\"></i> Editar.</a>"
                . "<a   onclick=\"$.VerInhu('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                . "<i class=\"fa fa-search\"></i> Ver</a>"
                . "<a onclick=\"$.deletInhu('" . $cod . "')\" class=\"btn default btn-xs red\">"
                . "<i class=\"fa fa-trash-o\"></i> Eliminar.</a>"
                . "<a onclick=\"$.PrintInhu('" . $cod . "')\" class=\"btn default btn-xs blue-dark\">"
...
$consulta = "SELECT count(*) as conta FROM exhumaciones WHERE estado='ACTIVO'";
