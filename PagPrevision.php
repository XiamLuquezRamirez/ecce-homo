...
$cad = "<table class=\"table table-striped table-bordered table-advance table-hover\">"
        ...
        . "</th>"
        . "<th>"
        . "<i class=\"fa fa-angle-right\"></i> Plan"
        . "</th>"
        . "<th>"
        . "<i class=\"fa fa-angle-right\"></i> Val. Cuota Mensual"
        . "</th>"
        . "<th>"
        . "<i class=\"fa fa-angle-right\"></i> Val. Anual"
        . "</th>"
        . "<th>"
...
if ($busq != "") {
    ...
    $consulta = "SELECT id_contr,ncontrato,fecha_cre,plan, CONCAT(id_titu,' ',nomb_titu) titu,val_anual,val_mes,saldo FROM  contrato_prevision  WHERE ";
    ...
    $consulta = "SELECT id_contr,ncontrato,fecha_cre,plan, CONCAT(id_titu,' ',nomb_titu) titu,val_anual,val_mes,saldo FROM contrato_prevision WHERE  estado='ACTIVO' order by nomb_titu ASC  LIMIT " . $regemp . "," . $regmos;
}
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_array($resultado)) {
        ...
        $cad .= "<tr>"
                ...
                . "</td>"
                . "<td class=\"highlight\">"
                . "<a  onclick=\"$.editPrev('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                . "<i class=\"fa fa-edit\"></i> Editar</a>"
                . "<a   onclick=\"$.VerPrev('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                . "<i class=\"fa fa-search\"></i> Ver</a>"
                . "<a onclick=\"$.deletPrev('" . $cod . "')\" class=\"btn default btn-xs red\">"
                . "<i class=\"fa fa-trash-o\"></i> Eliminar</a>"
                . "<a onclick=\"$.PrintPrev('" . $cod . "')\" class=\"btn default btn-xs blue-dark\">"
...
$consulta = "SELECT count(*) as conta FROM contrato_prevision WHERE estado='ACTIVO'";
