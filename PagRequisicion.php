...
$cad = "<table class=\"table table-striped table-bordered table-advance table-hover\">"
        ...
        . "</th>"
        . "<th>"
        . "<i class=\"fa fa-angle-right\"></i> Funeraria"
        . "</th>"
        . "<th>"
...
if ($busq != "") {
    ...
    $consulta = "SELECT id_req,cod_req,idfall_req,fech_req,nomfall_req,ced_contra,nom_contr,salve_req, nom_fune FROM  requisiciones req LEFT JOIN funerarias fun ON req.funau_req=fun.id_fune  WHERE ";
    ...
    $consulta .= " AND estado='ACTIVO' order by nomfall_req ASC LIMIT " . $regemp . "," . $regmos;
} else {

    $consulta = "SELECT id_req,cod_req,idfall_req,fech_req,nomfall_req,ced_contra,nom_contr,salve_req, nom_fune FROM  requisiciones req LEFT JOIN funerarias fun ON req.funau_req=fun.id_fune WHERE  estado='ACTIVO' order by nomfall_req ASC  LIMIT " . $regemp . "," . $regmos;
}
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_array($resultado)) {
        ...
        $cad .= "<tr>"
                ...
                . "</td>"
                . "<td class=\"highlight\">"
                . acentos($fila["nom_fune"]) . ""
                . "</td>"
                . "<td class=\"highlight\">"
                . acentos($fila["idfall_req"] . " - " . $fila["nomfall_req"]) . ""
                . "</td>"
                . "<td class=\"highlight\">"
                . acentos($fila["ced_contra"] . " - " . $fila["nom_contr"]) . ""
                . "</td>"
                . "<td class=\"highlight\">"
                . "<a  onclick=\"$.editReq('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                . "<i class=\"fa fa-edit\"></i> Editar.</a>"
                . "<a   onclick=\"$.VerReq('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                . "<i class=\"fa fa-search\"></i> Ver</a>"
                . "<a onclick=\"$.deletReq('" . $cod . "')\" class=\"btn default btn-xs red\">"
                . "<i class=\"fa fa-trash-o\"></i> Eliminar.</a>"
                . "<a onclick=\"$.PrintReq('" . $cod . "')\" class=\"btn default btn-xs blue-dark\">"
                . "<i class=\"fa fa-file-pdf-o\"></i> Imprimir.</a>";
                if ($_SESSION['gesFact'] == "s") {
                    $cad .= "<a onclick=\"$.FactReq('" . $cod . "')\" class=\"btn default btn-xs blue-dark\">"
                            . "<i class=\"fa fa-plus\"></i> Factura.</a>";
                }
...
$consulta = "SELECT count(*) as conta FROM requisiciones WHERE estado='ACTIVO'";
